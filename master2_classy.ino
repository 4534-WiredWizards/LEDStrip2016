...
void loop() {
  ...
  switch (animationID){
      ...
      case 7:
        ...
        break;
      case 8:
         strip1.rainbowBounce ();
         strip2.rainbowBounce ();
         strip3.rainbowBounce ();
...
for(int q = 0; q < 64; q += 1){
  for(int i = 0; i < this->numPixels(); i+=1){
    this->setPixelColor(i, this->Color((q*r)/50,(q*g)/50,(q*b)/50));
  }
  this->show();
  delay(wait);
  }
...
void NewNeoPixel::ripple(int r, int g, int b, byte magnitude, byte wait){
  ...
  int s = (f % (magnitude * 2)) - (magnitude * 2);
    ...
    for (int p = 0; p < this->numPixels() + (2 * magnitude); p += (2*magnitude)) {
...
void NewNeoPixel::rainbowBounce() {
    /*
    if (Serial.available() > 0) {
      incoming = Serial.read(); // reads the next thing sent on the serial line.
      if (incoming == 'a') {
